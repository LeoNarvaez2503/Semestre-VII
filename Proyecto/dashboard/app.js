const API_ESPACIOS = 'http://localhost:9000/espacio/listar'; // Redirigido a través del Gateway Kong
    ...
    const html = espacios.map((esp) => {
        ...
        return `
            <div class="espacio-card ${estadoClass} rounded-lg shadow p-4 flex flex-col">
                <div class="font-bold text-lg text-gray-800">${esp.name || esp.nombre || 'Sin nombre'}</div>
                <div class="text-sm text-gray-600">Zona: ${esp.nombreZona || 'N/A'}</div>
                <div class="text-sm text-gray-600">Tipo: ${esp.type || esp.tipo || 'N/A'}</div>
