...
* how AMMs like Uniswap v2, Sushiswap, and Balancer 1 determine the prices they quote; and
...
A liquidity pool is a smart contract that holds reserves of two or more tokens and allows anyone to deposit and withdraw funds from them, but only according to very specific rules. One such rule is the **constant product formula x * y = k**, where x and y are the reserves of two tokens, A and B. In order to withdraw some amount of token A, one must deposit a proportional amount of token B to maintain the constant k before fees.
...
![Chart 1: Different AMM formulas result in different pricing curves](marcador_de_imagen)
...
![Chart 2: Price impact of different trade sizes](marcador_de_imagen)
*Price impact of different trade sizes*. *Chart 2: Comparing the average fill price (left y-axis) and price impact (right y-axis) for different order sizes (x-axis). Both factors increase with order size. The larger the order relative to the pool, the further above the market price will the trade get filled.*

![Chart 3: Price impact of sell order on different pool sizes](marcador_de_imagen)
*Price impact of sell order on different pool sizes*. *Chart 3: Comparing the average fill price (left y-axis) and price impact (right y-axis) of a 10 WBTC sell order on different pool sizes on Uniswap V2 (x-axis). The pool size is the total value of the pool including the reserves of both assets. The orders represent 0.19%, 1.85%, and 18.52% of the pool respectively. So a good rule of thumb is that the price impact of your order is about twice the size of your order relative to the pool.*
...
#### Notes
1. Other forms of DEXs based on a central-limit order book (such as Serum) or batch auctions (such as Gnosis) are not in scope for this article.
2. Fees slightly increase the k every trade.
