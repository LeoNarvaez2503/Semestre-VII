...
class RoleService:
    ...
    def create_role(db: Session, role_in: RoleCreate) -> Role:
        ...
        existing_role = db.query(Role).filter(func.lower(Role.name) == func.lower(role_in.name)).first()
        if existing_role:
            if existing_role.active:
                raise EntityAlreadyExistsException(f"El rol con nombre '{func.lower(role_in.name)}' ya está registrado.")
            else:
