...
let AuditoriaInterceptor = class AuditoriaInterceptor {
    logger = new common_1.Logger('AuditoriaInterceptor');
        ...
        return next.handle().pipe((0, operators_1.tap)({
            next: (responseData) => {
                ...
                if (['POST', 'PATCH', 'DELETE'].includes(method)) {
                    this.logger.log(`[${method}] ${url} — ${duration}ms — Operación de asignación completada exitosamente`);
                }
            },
            error: (error) => {
                const duration = Date.now() - now;
                this.logger.warn(`[${method}] ${url} — ${duration}ms — Error: ${error.message}`);
            },
        }));
    }
};
